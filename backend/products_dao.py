...
def update_product(connection, updated_product):
    cursor = connection.cursor()
    query = ("UPDATE products "
             "SET name = %s, uom_id = %s, price_per_unit = %s "
             "WHERE product_id = %s")
    data = (updated_product['product_name'], updated_product['uom_id'], updated_product['price_per_unit'], updated_product['product_id'])
